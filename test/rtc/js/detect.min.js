...
function checkBrowser(browserList, current) {
  for (var i = 0; i < browserList.length; ++i) {
    if (current.name == browserList[i].name) {
      if (current.major >= browserList[i].version) {
        return true;
      } else {
  ...
  switch (options.name) {
    case 'Windows':
      if (/^\d*/.exec(options.version)[0] >= 7) {
        return checkBrowser(BROWSER.Win, ua.getBrowser());
      } else {
    ...
    case 'iOS':
      if (/^\d*/.exec(options.version)[0] >= 11) {
        return checkBrowser(BROWSER.iOS, ua.getBrowser());
      } else {
        return ERRORCODE.SYSTEM_VER_ERROR;
      }
      break;
    case 'Android':
      if (/^\d*/.exec(options.version)[0] >= 4) {
        return checkBrowser(BROWSER.Android, ua.getBrowser());
      } else {
        return ERRORCODE.SYSTEM_VER_ERROR;
      }
      break;
    case 'Mac OS':
      if (/^\d*/.exec(options.version)[0] >= 10) {
        return checkBrowser(BROWSER.macOS, ua.getBrowser());
      } else {
        return ERRORCODE.SYSTEM_VER_ERROR;
      }
      break;
